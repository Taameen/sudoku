...
#                           main.py
#
# Полная реализация генератора Судоку с документацией, 
# ASCII-визуализацией и экспортом в HTML для печати.
##################################################################


def _solve_sudoku(grid_size, board):
    """Решатель судоку, возвращающий генератор всех возможных решений."""
...
def generate_algorithm_doc(filename="ALGORITHM.md"):
    """Создает файл с подробным описанием правил и работы алгоритма."""
...
class Grid(object):
    """Генератор и манипулятор сеткой Судоку."""
    ...
    def to_string(self, empty_char="."):
        """Возвращает строковое представление сетки (ASCII-арт)."""
        n = self.__n
        grid_dim = n * n
    ...
    def show(self):
        """Выводит красивую текстовую сетку в консоль."""
        self._print(self.to_string())

    def transpose(self):
        """Транспонирование сетки."""
        self.__table = [list(x) for x in zip(*self.__table)]

    def swap_rows(self):
        """Перестановка двух строк в пределах одного блока."""
    ...
    def swap_columns(self):
        """Перестановка двух столбцов в пределах одного блока."""
        self.transpose()
        self.swap_rows()
        self.transpose()

    def swap_rows_area(self):
        """Перестановка двух горизонтальных блоков."""
    ...
    def swap_columns_area(self):
        """Перестановка двух вертикальных блоков."""
        self.transpose()
        self.swap_rows_area()
        self.transpose()

    def mix(self, amt=10):
        """Применение случайных валидных преобразований к сетке."""
        mix_func = (
            self.transpose,
            self.swap_rows,
            self.swap_columns,
    ...
    def export_to_html(self, filename="sudoku.html", title="Головоломка Судоку"):
        ...
        for i in range(grid_dim):
            ...
            for j in range(grid_dim):
                val = self.__table[i][j]
                # Пустые ячейки помечаем классом 'empty' для стилизации
...
if __name__ == "__main__":
    ...
    # 1. Создаем файл с описанием алгоритма и правилами
    generate_algorithm_doc()
    
    # 2. Инициализируем сетку 3x3 (классическое судоку 9x9)
    example = Grid(n=3, print_callback=print)
    
    # 3. Перемешиваем
    print("\n[1/4] Перемешивание базовой сетки...")
    example.mix(amt=15)
    
    # 4. Показываем полное решение
    print("\n[2/4] Полное решение (для проверки):")
    example.show()
    
    # 5. Генерируем головоломку
    print("\n[3/4] Генерация головоломки (подбор единственного решения)...")
    removed_cells = example.generate_puzzle()
    
    # 6. Показываем итоговую головоломку
    print(f"\n[4/4] Итоговая головоломка (удалено ячеек: {removed_cells}):")
    example.show()
    
    # 7. Экспорт в HTML (не требует установки библиотек!)
    print("\n[ЭКСПОРТ] Создание файла для печати...")
